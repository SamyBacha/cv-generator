# Spec: mode-url-routing

## Purpose

The system SHALL keep the browser URL in sync with the active mode (viewer vs editor), restore the mode on page load from the URL, and support browser back/forward navigation between modes — all without triggering full page reloads.
